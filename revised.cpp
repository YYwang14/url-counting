...
void processChunk(const std::string& filename, std::streampos start, std::streampos end, 
    ...
    while (pos < end && std::getline(file, url)) {
        if (!url.empty()) {
            localFreq[url]++;
            lineCount++;
            if (lineCount % 1000000 == 0) {
                logMessage(logFile, "Processed " + std::to_string(lineCount) + " lines", logMutex);
            }
        }
        pos = file.tellg();
    }
    ...
    try {
        ...
        for (size_t i = 0; i < NUM_THREADS; ++i) {
            threads.emplace_back(processChunk, filename, chunkStarts[i], chunkEnds[i], 
                               std::ref(totalFreq), std::ref(mapMutex),
                               std::ref(logFile), std::ref(logMutex));
        }
        ...
        logMessage(logFile, "Process completed. Total time: " + std::to_string(duration) + " seconds", logMutex);
